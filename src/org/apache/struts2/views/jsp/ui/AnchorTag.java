...
public class AnchorTag extends AbstractClosingTag {
	...
	public int doEndTag() throws JspException {
    	User user = (User) pageContext.getSession().getAttribute("user"); // 得到当前登录用户
    	String url = action;        								      // 得到当前标签对应的url
    	url = '/' + url;												  // 为url加上namespace
    	if(user.hasPrivilgeByUrl(url)) {
    		return super.doEndTag();		// 正常生成显示标签，继续执行页面后面的代码
    	} else{
    		return EVAL_PAGE;				// 不生成显示标签，继续执行页面后面的代码
    	}
